    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"첫번째 메모는 아이디어 주제에 대한 것이고 두번째 메모부터 아이디어 내용을 구체화 하는 메모입니다. 아이디어 계획 형태(아이디어 흐름을 번호로 정리해주세요)를 형식에 맞춰서 정리해주세요. 그리고 첫번째 메모에 적혀진 주제에 적힌 내용와 나머지 메모에서 아이디어 내용이 어긋나는 것이 있으면 이 내용이 주제와 벗어났다고 알려주세요:\n\n{all_text}"
                }
            ],
            max_tokens=300,
            temperature=0.7,
